def main():
    """
    DANSE 训练/测试主入口，支持自动数据集分割、目录检测、实验日志保存。
    ---
    ...
    batch_size = est_parameters_dict["danse"]["batch_size"]
    ...
    '''
    ./log/
    └── LorenzSSM_danse_gru_m_3_n_3_T_200_N_1000_40.0dB_0.0dB/
        ├── training.log
        └── testing.log

    ./models/
    └── LorenzSSM_danse_gru_m_3_n_3_T_200_N_1000_40.0dB_0.0dB/
        ├── danse_gru_ckpt_epoch_50.pt
        ├── danse_gru_ckpt_epoch_100.pt
        └── danse_gru_ckpt_epoch_200_best.pt
    ...
    if mode.lower() == "train":
        ...
        # 实例化 DANSE
        model_danse = DANSE(**estimator_options).to(device)

        # 从配置里取训练轮数
        rnn_type = estimator_options["rnn_type"]
        num_epochs = estimator_options["rnn_params_dict"][rnn_type]["num_epochs"]

        # 开始训练（旧版 train_danse 的签名是：model, options, train_loader, val_loader, nepochs, ...）
        tr_verbose = True
        save_chkpoints = "some"
        tr_losses, val_losses, _, _, _ = train_danse(
            model=model_danse,
        ...
        with open(os.path.join(logfile_path, main_exp_name,
                f'danse_{estimator_options["rnn_type"]}_losses_eps{num_epochs}.json'), 'w') as f:
            import json
            f.write(json.dumps({"tr_losses": tr_losses, "val_losses": val_losses}, cls=NDArrayEncoder, indent=2))

    elif mode.lower() == "test":
        # === 改动：旧版 test_danse 的签名是 (test_loader, options, device, model_file, test_logfile_path)
        from src.danse import test_danse  # ← 按你的项目路径调整

        te_loss = test_danse(
            test_loader=test_loader,
            options=estimator_options,          # === 改动：传 options（旧版 test 会内部构建 DANSE 并 load 权重）
            device=device,
            model_file=model_file_saved,
            test_logfile_path=te_logfile_name_with_path
        )
